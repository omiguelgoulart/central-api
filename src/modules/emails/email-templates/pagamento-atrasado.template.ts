export interface PagamentoAtrasadoProps {
    nome: string;
    valor: number;
    descricao: string;
...
export const pagamentoAtrasadoTemplate = ({
    nome,
    valor,
    descricao,
    dataVencimento,
    linkPagamento,
}: PagamentoAtrasadoProps): string => {
    const botaoPagar = linkPagamento
        ? `
            <div style="text-align: center; margin: 30px 0;">
                <a href="${linkPagamento}" style="background-color: #c41e3a; color: white; padding: 12px 30px; text-decoration: none; border-radius: 4px; font-weight: bold; display: inline-block;">
                    Pagar Agora
                </a>
            </div>
        `
    ...
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <div style="padding: 30px 20px;">
            ...
            <div style="background-color: #ffe6e6; border-left: 4px solid #cc0000; padding: 15px; margin: 20px 0;">
                <p style="margin: 0 0 10px 0; color: #7a0000; font-weight: bold;">Pagamento pendente:</p>
                <p style="margin: 4px 0; color: #555;">
                    <strong style="display: inline-block; width: 100px;">Descrição:</strong> ${descricao}
                </p>
                <p style="margin: 4px 0; color: #555;">
                    <strong style="display: inline-block; width: 100px;">Valor:</strong> R$ ${valor.toFixed(2)}
                </p>
                <p style="margin: 4px 0; color: #555;">
                    <strong style="display: inline-block; width: 100px;">Vencimento:</strong> ${dataVencimento}
                </p>
            </div>
