...
router.post("/", async (req, res) => {
    try {
        const { jogoId, loteId, valor } = ingressoCreateSchema.parse(req.body);
        ...
        if (err?.message === "Valor inválido") return res.status(400).json({ error: "Valor inválido" });
        console.error(err);
...
router.put("/:id", async (req, res) => {
    try {
        ...
        if ("usadoEm" in data) payload.usadoEm = parseOptionalDate(data.usadoEm!) ?? null;
        if ("pagamentoId" in data) payload.pagamentoId = data.pagamentoId ?? null;
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        if (e instanceof z.ZodError) return res.status(400).json({ errors: e.errors });
        if (e?.code === "P2025") return res.status(404).json({ error: "Ingresso não encontrado" });
        console.error(e);
